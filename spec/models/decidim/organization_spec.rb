    ...
    describe "Organization" do
      ...
      describe "method with default values" do
        it "should do something" do
          ...
          expect(current_organization.can_user_send_private_message?).to eq(true)
      ...
      describe "method with changed values" do
        before do
          current_organization.privacy_setting.update(user_avatar: false, user_follow: false, user_message: false)
        end

        it "should do something" do
          expect(current_organization.can_upload_avatar?).to eq(false)
          expect(current_organization.can_user_follow?).to eq(false)
          expect(current_organization.can_user_send_private_message?).to eq(false)
          expect(current_organization.privacy_settings).not_to be_empty
        end
      end
